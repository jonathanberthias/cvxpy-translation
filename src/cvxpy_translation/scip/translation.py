...
def promote_array_to_scip_matrixapi(array: npt.NDArray[np.object_]) -> Any:
    ...
    if issubclass(kind, scip.Var):
        return scip.MVar.fromlist(array)
...
def translate_variable(var: cp.Variable, model: scip.Model) -> AnyVar:
    lb = -scip.GRB.INFINITY
    ub = scip.GRB.INFINITY
    if var.is_nonneg():
        lb = 0
    if var.is_nonpos():
        ub = 0

    vtype = scip.CONTINUOUS
...
def _should_reverse_inequality(lower: object, upper: object) -> bool:
    ...
    we get an error if lower is an array and upper is a scip object:
        scip.ScipError:
            Constraint has no bool value (are you trying "lb <= expr <= ub"?)
    In that case, we should write upper >= lower instead.
    """
    # scip objects don't have base classes and don't define __module__
    # This is very hacky but seems to work
    upper_from_scip = "'scip." in str(type(upper))
...
class Translater:
    ...
    def translate_into_variable(
        ...
        *,
        ...
        This is useful for SCIP functions that only handle variables as their arguments.
        ...
        """
        expr = self.visit(node)
        if isinstance(expr, scip.Var):
            return expr
        if isinstance(expr, (scip.MVar, np.ndarray)):
            if scalar:
    ...
    def _stack(self, node: Hstack | Vstack, scip_fn: Callable) -> Any:
        args = node.args
        exprs = [self.visit(arg) for arg in args]
        return scip_fn(exprs)

    def visit_Hstack(self, node: Hstack) -> Any:
        if SCIP_VERSION < (11,):
            raise UnsupportedExpressionError(node)
        return self._stack(node, scip.hstack)
    ...
    def visit_Maximize(self, objective: cp.Maximize) -> None:
        ...
        self.model.setObjective(obj, sense=scip.MAXIMIZE)

    def visit_Minimize(self, objective: cp.Minimize) -> None:
        obj = self.translate_into_scalar(objective.expr)
        self.model.setObjective(obj, sense=scip.MINIMIZE)
    ...
    def visit_quad_over_lin(self, node: quad_over_lin) -> Any:
        ...
        quad = scip.quicksum(squares)
        lin = self.visit(y)
        return quad / lin

    def visit_reshape(self, node: cp.reshape) -> scip.MVar | npt.NDArray[np.float64]:
        ...
        expr = self.visit(x)
        if isinstance(expr, scip.Var):
            expr = scip.MVar.fromvar(expr)
        elif not isinstance(expr, scip.MVar):
            ...
            assert isinstance(expr, scip.MVar)
    ...
    def visit_Vstack(self, node: Vstack) -> Any:
        if SCIP_VERSION < (11,):
            raise UnsupportedExpressionError(node)
        return self._stack(node, scip.vstack)
