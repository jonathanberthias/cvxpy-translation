...
def reshape() -> Iterator[cp.Problem]:
    ...
    a = x + 1
    yield cp.Problem(cp.Minimize(x), [cp.reshape(x, ()) <= 1])
    yield cp.Problem(cp.Minimize(x), [cp.reshape(x, 1) <= np.ones(1)])
    yield cp.Problem(cp.Minimize(x), [cp.reshape(x, -1) <= np.ones(1)])
    yield cp.Problem(cp.Minimize(x), [cp.reshape(a, ()) <= 1])
    yield cp.Problem(cp.Minimize(x), [cp.reshape(a, (1,)) <= np.ones(1)])
    yield cp.Problem(cp.Minimize(x), [cp.reshape(a, -1) <= np.ones(1)])
    ...
    x = cp.Variable(2, name="x", nonneg=True)
    a = x + np.array([1, 1])
    yield cp.Problem(cp.Minimize(cp.sum(x)), [cp.reshape(x, (2,)) <= np.ones(2)])
    yield cp.Problem(cp.Minimize(cp.sum(x)), [cp.reshape(x, -1) <= np.ones(2)])
    yield cp.Problem(cp.Minimize(cp.sum(x)), [cp.reshape(x, (2, 1)) <= np.ones((2, 1))])
    yield cp.Problem(cp.Minimize(cp.sum(x)), [cp.reshape(x, (1, 2)) <= np.ones((1, 2))])
    yield cp.Problem(cp.Minimize(cp.sum(x)), [cp.reshape(a, (2,)) <= np.ones(2)])
    yield cp.Problem(cp.Minimize(cp.sum(x)), [cp.reshape(a, (-1,)) <= np.ones(2)])
    yield cp.Problem(cp.Minimize(cp.sum(x)), [cp.reshape(a, (2, 1)) <= np.ones((2, 1))])
    yield cp.Problem(cp.Minimize(cp.sum(x)), [cp.reshape(a, (1, 2)) <= np.ones((1, 2))])

    x = cp.Variable(4, name="x", nonneg=True)
    a = x + np.ones(4)
    yield cp.Problem(cp.Minimize(cp.sum(x)), [cp.reshape(x, (4,)) <= np.ones(4)])
    yield cp.Problem(cp.Minimize(cp.sum(x)), [cp.reshape(x, (4, 1)) <= np.ones((4, 1))])
    yield cp.Problem(cp.Minimize(cp.sum(x)), [cp.reshape(x, (2, 2)) <= np.ones((2, 2))])
    yield cp.Problem(cp.Minimize(cp.sum(x)), [cp.reshape(x, (1, 4)) <= np.ones((1, 4))])
    yield cp.Problem(cp.Minimize(cp.sum(x)), [cp.reshape(a, (4,)) <= np.ones(4)])
    yield cp.Problem(cp.Minimize(cp.sum(x)), [cp.reshape(a, (4, 1)) <= np.ones((4, 1))])
    yield cp.Problem(cp.Minimize(cp.sum(x)), [cp.reshape(a, (2, 2)) <= np.ones((2, 2))])
    yield cp.Problem(cp.Minimize(cp.sum(x)), [cp.reshape(a, (1, 4)) <= np.ones((1, 4))])
